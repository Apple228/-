...
vector<vector<pair<int, int>>> test_graph1()
{
    ...
    res[0].push_back(make_pair(1, 10));
    res[0].push_back(make_pair(3, 11));

    res[1].push_back(make_pair(0, 10));
    res[1].push_back(make_pair(2, 9));
    res[1].push_back(make_pair(3, 5));

    res[2].push_back(make_pair(1, 9));
    res[2].push_back(make_pair(3, 8));


    res[3].push_back(make_pair(0, 11));
    res[3].push_back(make_pair(1, 5));
    res[3].push_back(make_pair(2, 8));
    return res;
}
...
int main(int argc, const char* argv[])
{
    ...
    vector<vector<pair<int, int>>> graph = test_graph1();

    print(graph);
    unsigned int start_time = clock(); // начальное время
    algorithm_prima(graph, 4);
